...
class XiaomiCloudConnector {
  constructor(logger, authObj) {
    ...
    this.session = (0, import_axios_cookiejar_support.wrapper)(
      axios.create({
        jar: this.jar,
  ...
  init(authObj) {
    if (authObj) {
      ...
      if (authObj.jarData) {
        try {
          this.jar = import_tough_cookie.CookieJar.fromJSON(authObj.jarData);
          this.session = (0, import_axios_cookiejar_support.wrapper)(
            axios.create({
              jar: this.jar,
              withCredentials: true,
  ...
  async loginStep2() {
    ...
    const captchaCode = typeof this.captCode === "string" && this.captCode && this.captCode !== "true" ? String(this.captCode).trim() : null;
    if (captchaCode) {
  ...
  async startTwoFactor(notificationUrl) {
    ...
    try {
      ...
      const hint = sendJson && (sendJson.maskedEmailAddress || sendJson.maskedEmail || sendJson.maskedPhoneNumber || sendJson.maskedPhone) || (method === "Email" ? "your email" : "your phone");
      this.logger.info(
  ...
  async continueWith2FA(ticket) {
    ...
    try {
      ...
      if (endResp.status === 200 && typeof endResp.data === "string" && endResp.data.includes("Xiaomi Account - Tips")) {
      ...
      this.serviceToken = this.getCookie("serviceToken") || this.extractServiceTokenFromHeaders(stsResp.headers["set-cookie"]);
  ...
  async getDevices(country, homeIds) {
    ...
    if (!homeIds) {
      ...
      homeIds = (_a = this.homeIds) == null ? void 0 : _a.slice();
  ...
  signedNonce(nonce, ssecurity) {
    const hash = crypto.createHash("sha256").update(Buffer.concat([Buffer.from(ssecurity, "base64"), Buffer.from(nonce, "base64")])).digest();
    return Buffer.from(hash).toString("base64");
  }
  generateNonce(millis) {
    const randomBytes = crypto.randomBytes(8);
    const timeBytes = Buffer.alloc(4);
    timeBytes.writeUInt32BE(Math.floor(millis / 6e4), 0);
