...
class TimerManager {
  ...
  check() {
    if (this.nextProcessTime > 0 && this.nextProcessTime < /* @__PURE__ */ new Date()) {
      const diff = (/* @__PURE__ */ new Date()).getTime() - this.nextProcessTime.getTime();
      if (diff > 36e5) {
        adapter.log.info("timer was to old, skipped");
        timerManager.calcNextProcess();
      } else {
        adapter.log.debug("timer will trigger soon...");
        this.nextProcessTime = new Date(this.nextProcessTime.getTime() + 36e5);
        setTimeout(() => {
          adapter.log.info(`start cleaning by timer ${timerManager.nextTimerId}`);
          adapter.setForeignState(
            timerManager.nextTimerId,
            TimerManager.START,
            false,
            (err, obj) => (
  ...
  calcNextProcess() {
    const now = new Date((/* @__PURE__ */ new Date()).getTime() + 6e4);
    timerManager.nextProcessTime = new Date(now.getTime() + 6048e5);
    ...
    adapter.getStatesOf("timer", (err, timerObjects) => {
      try {
        ...
        adapter.getStates("timer.*", function(err2, timerStates) {
          for (const t in timerStates) {
            if (timerStates[t] !== null && timerStates[t].val != TimerManager.DISABLED) {
              if (timerStates[t].val == TimerManager.SKIP) {
                timers[t].time = timerManager._calcNextProcessTime(
                  timers[t].obj,
          ...
          const nextTimerName = timerManager.nextTimerId && timerManager.nextProcessTime ? `${i18n.weekDaysFull[timerManager.nextProcessTime.getDay()]} ${adapter.formatDate(
            timerManager.nextProcessTime,
            "hh:mm"
          )}` : i18n.notAvailable;
          const timerFolder = {
            id: `${adapter.namespace}.timer`,
            type: "channel",
            native: {},
            common: { name: `${i18n.nextTimer}: ${nextTimerName}` }
          };
          timerManager.nextProcessTime = new Date(
            timerManager.nextProcessTime.getTime() - adapter.config.pingInterval
          );
