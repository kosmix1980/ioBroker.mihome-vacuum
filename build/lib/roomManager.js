...
class RoomManager {
  ...
  processRoomMaping(response) {
    ...
    adapter.getChannelsOf("rooms", function(err, roomObjs) {
        ...
        for (const r in roomObjs) {
          ...
          if (extRoomId.indexOf("manual_") === -1) {
            ...
            if (!room) {
              ...
              adapter.setStateChanged(`${roomObj._id}.mapIndex`, roomNo, true, (err2, id, notChanged) => {
                if (!notChanged) {
      ...
      for (const extRoomId in rooms) {
        adapter.getObject(`rooms.${extRoomId}`, function(err2, roomObj) {
          if (roomObj) {
  ...
  cleanRooms(mapIndexStates) {
    adapter.getForeignStates(mapIndexStates, function(err, states) {
      ...
      if (states) {
        ...
        if (zones.length > 0) {
          adapter.sendTo(adapter.namespace, "cleanZone", { zones, channels: zoneChannels });
        }
      }
    });
  }
  ...
  cleanRoomsFromState(id) {
    adapter.getForeignObjects(id, "state", "rooms", (err, states) => {
      if (states && states[id].native) {
        ...
        if (rooms.length > 0) {
          roomManager.findMapIndexByRoom(rooms, (states2) => roomManager.cleanRooms(mapIndex.concat(states2)));
      ...
      if (states) {
        ...
        for (const stateId in states) {
          for (const r in states[stateId].enums) {
            if (rooms.indexOf(r) >= 0 && stateId.indexOf(".mapIndex") > 0) {
    ...
    adapter.createChannel("rooms", roomId, (err, roomObj) => {
      if (roomObj) {
        ...
        adapter.setObjectNotExists(
          ...
          (err2) => !err2 && adapter.setState(`${roomObj.id}.mapIndex`, mapIndex, true)
        );
  ...
  updateRoomStates(roomObj_id) {
    ...
    adapter.getObject("control.water_box_level", (err, obj) => {
      obj && adapter.getState(obj._id, () => {
        adapter.setObjectNotExists(
