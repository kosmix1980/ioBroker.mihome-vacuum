...
class MapHelper {
  ...
  getRawMapData(urlstring) {
    ...
    return new Promise(function(resolve, reject) {
      axios.get(url, {
        ...
        // wichtig für Binärdaten
        decompress: false
        // wir entpacken manuell
      }).then((response) => {
  ...
  getMapBase64(url) {
    return new Promise((resolve, reject) => {
      ...
      this.getRawMapData(url).then((data) => {
        try {
  ...
  login() {
    return Promise.resolve(this._sessionReady).then(() => this.cloudConnector.login()).then(async (result) => {
      if (result && result.ok) {
        if (result.session) {
    ...
    return new Promise((resolve, reject) => {
      if (this.config.mimap === true) {
        ...
        if (!mapUrlCache[mapurl] || mapUrlCache[mapurl].expires < unixTime - 60) {
          ...
          this.getMapURL(mapurl).then((result) => {
            mapUrlCache[mapurl] = {
            ...
            this.getMapBase64(mapUrlCache[mapurl].url).then((mapData) => resolve(mapData)).catch((error) => reject(error));
          }).catch((error) => {
            this.adapter.log.warn(`map error:${error}`);
            if (!dontRetry) {
              this.login().then(() => this.updateMap(mapurl, true)).catch((error2) => reject(error2));
            }
          });
        } else {
        ...
        this.getMapBase64().then((mapData) => resolve(mapData)).catch((error) => reject(error));
      }
    });
  }
  getMapURL(mapName) {
    return new Promise(
      (resolve, reject) => this.login().then(() => {
        let url;
        ...
        this.cloudConnector.executeEncryptedApiCall(url, { data }).then((json) => {
          try {
            if (json.message === "ok") {
              resolve(json);
            } else {
