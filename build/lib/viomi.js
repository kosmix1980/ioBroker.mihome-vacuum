...
class ViomiManager {
  ...
  async getStates() {
    ...
    if (DeviceData && DeviceData.result) {
      ...
      answer.forEach((element, index) => {
        const objExist = objects.viomiObjects.find((element2) => element2._id === this.PARAMS[index]);
  ...
  async stateChange(id, state) {
    ...
    try {
      ...
      adapter.log.warn(`Cant send command please try again${command} - ${error}`);
    }
  }
  startClean() {
  }
