...
let canvasImg = new import_canvas.Canvas();
const img = new import_canvas2.Image();
img.src = robot2;
const img_charger = new import_canvas2.Image();
img_charger.src = charger;
const MapCreator = () => {
};
MapCreator.rotateCanvas = function(img2, angle) {
  const canvasImg2 = (0, import_canvas.createCanvas)(100, 100);
  const ctxImg = canvasImg2.getContext("2d");
  const offset2 = 90;
  ctxImg.clearRect(0, 0, 100, 100);
  ctxImg.translate(100 / 2, 100 / 2);
  ctxImg.rotate((angle + offset2) * Math.PI / 180);
  ctxImg.translate(-100 / 2, -100 / 2);
  ctxImg.drawImage(img2, 100 / 2 - img2.width / 2, 100 / 2 - img2.height / 2);
  return canvasImg2;
};
MapCreator.CanvasMap = (Mapdata, options, adapter) => {
  ...
  let minleft = 0;
  let mintop = 0;
  const canvas = (0, import_canvas.createCanvas)();
      ...
      ["floor", "obstacle"].forEach((key) => {
        ...
        Mapdata.image.pixels[key].forEach(function drawPixel(px) {
          ...
          ctx.rect(
            ...
            (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 + Mapdata.image.position.top * 4,
            4,
            4
          );
          maxtop = Math.max(
  ...
  if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
    ...
    Mapdata.image.pixels.carpet.forEach((px) => {
      ...
      ctx.lineWidth = 1;
      ctx.moveTo(
        px % Mapdata.image.dimensions.width * 4 + Mapdata.image.position.left * 4 + offset2,
        (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 + Mapdata.image.position.top * 4
      );
      ctx.lineTo(
        px % Mapdata.image.dimensions.width * 4 + Mapdata.image.position.left * 4 + offset2 + 4,
        (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 + Mapdata.image.position.top * 4 + 4
      );
    });
    ctx.stroke();
  }
  ...
  const canvas_final = (0, import_canvas.createCanvas)();
