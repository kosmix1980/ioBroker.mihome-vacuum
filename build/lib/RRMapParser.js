...
RRMapParser.PARSEBLOCK = function parseBlock(buf, offset, result) {
  ...
  let g3offset = 0;
  const type = buf.readUInt16LE(0 + offset);
  const hlength = buf.readUInt16LE(2 + offset);
  const length = buf.readUInt32LE(4 + offset);
  switch (type) {
    case RRMapParser.TYPES.ROBOT_POSITION:
    case RRMapParser.TYPES.CHARGER_LOCATION:
      result[type] = {
        position: [buf.readUInt16LE(8 + offset), buf.readUInt16LE(12 + offset)],
        angle: length >= 12 ? buf.readInt32LE(16 + offset) : 0
        // gen3+
      };
      break;
    case RRMapParser.TYPES.IMAGE: {
      ...
      const parameters = {
        segments: {
          count: g3offset ? buf.readInt32LE(8 + offset) : 0,
          id: []
        },
        position: {
          top: buf.readInt32LE(8 + g3offset + offset),
          left: buf.readInt32LE(12 + g3offset + offset)
        },
        dimensions: {
          height: buf.readInt32LE(16 + g3offset + offset),
          width: buf.readInt32LE(20 + g3offset + offset)
        },
        pixels: []
      };
      ...
      if (parameters.dimensions.height > 0 && parameters.dimensions.width > 0) {
        ...
        for (let s, i = 0; i < length; i++) {
          switch (buf.readUInt8(24 + g3offset + offset + i) & 7) {
            case 0:
              break;
            case 1:
            ...
            default:
              ...
              s = (buf.readUInt8(24 + g3offset + offset + i) & 248) >> 3;
              if (s !== 0) {
    ...
    case RRMapParser.TYPES.CARPET_MAP: {
      ...
      for (let i = 0; i < length; i++) {
        switch (buf.readUInt8(24 + offset + i) & 7) {
          case 0:
            break;
          case 1:
            carpet.push(i);
    ...
    case RRMapParser.TYPES.MOP_PATH: {
      const mopactive = [];
      for (let i = 0; i < length; i++) {
        mopactive.push(buf.readUInt8(20 + offset + i));
      }
    ...
    case RRMapParser.TYPES.GOTO_PREDICTED_PATH: {
      ...
      for (let i = 0; i < length; i = i + 4) {
        points.push([buf.readUInt16LE(20 + offset + i), buf.readUInt16LE(20 + offset + i + 2)]);
      }
      result[type] = {
        //point_count: buf.readUInt32LE(0x08 + offset),
        //point_size: buf.readUInt32LE(0x0c + offset),
        current_angle: buf.readUInt32LE(16 + offset),
        //This is always 0. Roborock didn't bother
        points
      };
      break;
    }
    case RRMapParser.TYPES.GOTO_TARGET:
      result[type] = {
        position: [buf.readUInt16LE(8 + offset), buf.readUInt16LE(10 + offset)]
      };
      break;
    case RRMapParser.TYPES.CURRENTLY_CLEANED_ZONES: {
      const zoneCount = buf.readUInt32LE(8 + offset);
      const zones = [];
      if (zoneCount > 0) {
        for (let i = 0; i < length; i = i + 8) {
          zones.push([
            buf.readUInt16LE(12 + offset + i),
            buf.readUInt16LE(12 + offset + i + 2),
            buf.readUInt16LE(12 + offset + i + 4),
            buf.readUInt16LE(12 + offset + i + 6)
          ]);
        }
        result[type] = zones;
      }
    ...
    case RRMapParser.TYPES.FORBIDDEN_ZONES: {
      const forbiddenZoneCount = buf.readUInt32LE(8 + offset);
      ...
      if (forbiddenZoneCount > 0) {
        for (let i = 0; i < length; i = i + 16) {
          forbiddenZones.push([
            ...
            buf.readUInt16LE(12 + offset + i + 4),
            buf.readUInt16LE(12 + offset + i + 6),
            buf.readUInt16LE(12 + offset + i + 8),
            buf.readUInt16LE(12 + offset + i + 10),
            buf.readUInt16LE(12 + offset + i + 12),
            buf.readUInt16LE(12 + offset + i + 14)
          ]);
        }
    ...
    case RRMapParser.TYPES.VIRTUAL_WALLS: {
      const wallCount = buf.readUInt32LE(8 + offset);
      const walls = [];
      if (wallCount > 0) {
        for (let i = 0; i < length; i = i + 8) {
          walls.push([
    ...
    case RRMapParser.TYPES.CURRENTLY_CLEANED_BLOCKS: {
      const blockCount = buf.readUInt32LE(8 + offset);
      const blocks = [];
      if (blockCount > 0) {
        for (let i = 0; i < length; i++) {
          blocks.push(buf.readUInt8(12 + offset + i));
        }
        result[type] = blocks;
      }
      break;
...
RRMapParser.PARSE = function parse(mapBuf) {
  if (mapBuf && mapBuf[0] === 114 && mapBuf[1] === 114) {
    const parsedMapData = {
      header_length: mapBuf.readUInt16LE(2),
      data_length: mapBuf.readUInt16LE(4),
      version: {
        major: mapBuf.readUInt16LE(8),
        minor: mapBuf.readUInt16LE(10)
      },
      map_index: mapBuf.readUInt16LE(12),
      map_sequence: mapBuf.readUInt16LE(16)
    };
...
RRMapParser.PARSEDATA = function parseData(mapBuf) {
  ...
  const blocks = RRMapParser.PARSEBLOCK(mapBuf, 20);
  const parsedMapData = {};
      ...
      if (blocks[item.type]) {
        ...
        if (parsedMapData[item.path].points.length >= 2) {
          parsedMapData[item.path].current_angle = Math.atan2(
            parsedMapData[item.path].points[parsedMapData[item.path].points.length - 1][1] - parsedMapData[item.path].points[parsedMapData[item.path].points.length - 2][1],
            parsedMapData[item.path].points[parsedMapData[item.path].points.length - 1][0] - parsedMapData[item.path].points[parsedMapData[item.path].points.length - 2][0]
          ) * 180 / Math.PI;
        }
      }
    });
