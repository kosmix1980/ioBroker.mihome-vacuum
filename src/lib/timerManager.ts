        ...
        if (!nextProcessTime || nextProcessTime < now) {
            ...
            if (nextProcessTime && nextProcessTime != timerObj.native.nextProcessTime && !onlyCalc) {
                ...
                timerObj.common.states['1'] =
                    `${i18n.weekDaysFull[nextProcessTime.getDay()]} ${adapter.formatDate(nextProcessTime, 'hh:mm')}`;
                let name = '';
        ...
        adapter.getStatesOf('timer', (err, timerObjects) => {
            try {
                ...
                adapter.getStates('timer.*', function (err, timerStates) {
                    ...
                    const nextTimerName =
                        timerManager.nextTimerId && timerManager.nextProcessTime
                            ? `${i18n.weekDaysFull[timerManager.nextProcessTime.getDay()]} ${adapter.formatDate(
